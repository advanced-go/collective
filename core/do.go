...
func Do(req *http.Request) (resp *http.Response, status *Status) {
	if req == nil {
		return &http.Response{StatusCode: http.StatusInternalServerError}, NewStatusError(StatusInvalidArgument, errors.New("invalid argument : request is nil"))
	}
	...
	if err != nil {
		...
		return resp, NewStatusError(resp.StatusCode, err)
	}
	return resp, NewStatus(resp.StatusCode)
}
...
func fileSchemeStatus(resp *http.Response) *Status {
	...
	default:
		return NewStatusError(resp.StatusCode, errors.New(HttpStatus(resp.StatusCode)))
	}
}
